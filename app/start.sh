...
mkdir -p /data/kodi >/dev/null 2>&1 || true && rm -rf /root/.kodi && ln -s /data/kodi /root/.kodi && mkdir -p /data/kodi/temp
...
if [[ "$ENABLE_REMOTE_CONTROL_VIA_ADVANCEDSETTINGS" = "1" ]]; then
  ...
  mkdir -p /data/kodi/userdata >/dev/null 2>&1 || true && cp /usr/src/app/advancedsettings.xml /data/kodi/userdata
else
  # Remove advancedsettings.xml 
  mkdir -p /data/kodi/userdata >/dev/null 2>&1 || true && rm -rf /data/kodi/userdata/advancedsettings.xml
fi
